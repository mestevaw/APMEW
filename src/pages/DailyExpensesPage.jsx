...
const parseCSV = (text) => {
  ...
  for (let i = 0; i < Math.min(15, lines.length); i++) {
    ...
    if (lower.includes("date") && (lower.includes("description") || lower.includes("amount"))) { headerIdx = i; break; }
  }
  ...
  return lines.slice(headerIdx + 1).map(line => {
    ...
    return obj;
  }).filter(row => Object.values(row).filter(v => v).length >= 3);
};
...
const exportToExcel = (data) => {
  const BOM = "\uFEFF";
  const headers = "Fecha,Concepto,Categoría,Quién,Monto,Tarjeta,Asociado\n";
  const rows = data.map(e => `${e.expense_date},"${(displayConcept(e)).replace(/"/g,'""')}",${e.category},${e.who},${Number(e.amount).toFixed(2)},${e.source || ""},${e.tag || ""}`).join("\n");
...
export const DailyExpensesPage = ({ dailyExpenses, onAdd, mob, reload }) => {
  ...
  const [showTagging, setShowTagging] = useState(null); // expense id
  ...
  const handleFileSelect = async (e) => {
    ...
    } catch (err) { setImportMsg("Error: " + err.message); return; }
  ...
  // ─── Tag/Associate ───
  const handleTag = async (expenseId, tag) => {
    try { await supaUpdate("daily_expenses", expenseId, { tag }); reload(); } catch (e) { console.error(e); }
    setShowTagging(null);
  };
  ...
  if (search) {
    ...
    filtered = filtered.filter(e => (e.concept || "").toLowerCase().includes(q) || (e.category || "").toLowerCase().includes(q) || (e.source || "").toLowerCase().includes(q) || (e.tag || "").toLowerCase().includes(q) || (e.who || "").toLowerCase().includes(q));
  }
  ...
  return (
    <div>
        ...
        <div style={{ position: "relative" }}>
          ...
          <DropMenu open={menuOpen} onClose={() => setMenuOpen(false)}>
            ...
            {Object.entries(sortLabels).map(([col, label]) => (
              <MenuBtn key={col} onClick={() => doSort(col)} active={sortCol === col}>{label} {sortCol === col && (sortDir === "asc" ? "▲" : "▼")}</MenuBtn>
            ))}
            <MenuDivider />
            <MenuLabel>Excel</MenuLabel>
            <MenuBtn onClick={() => { setShowImport(true); setShowForm(false); setShowSummary(false); setShowQuery(false); setMenuOpen(false); }}>📥 Importar</MenuBtn>
            <MenuBtn onClick={() => { exportToExcel(sorted); setMenuOpen(false); }}>📊 Exportar</MenuBtn>
            <MenuBtn onClick={() => { setShowForm(true); setShowImport(false); setShowSummary(false); setShowQuery(false); setMenuOpen(false); }}>✏️ Registrar gasto</MenuBtn>
            <MenuDivider />
            <MenuBtn onClick={() => { setShowSummary(!showSummary); setShowForm(false); setShowImport(false); setShowQuery(false); setMenuOpen(false); }}>📋 Resumen</MenuBtn>
            <MenuBtn onClick={() => { setShowQuery(!showQuery); setShowForm(false); setShowImport(false); setShowSummary(false); setMenuOpen(false); }}>🔍 Consulta</MenuBtn>
          </DropMenu>
        </div>
      </div>
      ...
      <div style={{ marginTop: 10, marginBottom: 10 }}>
        <input
          placeholder="🔍 Buscar por concepto, categoría, tarjeta, tag..."
          value={search} onChange={e => setSearch(e.target.value)}
          style={{ ...inputStyle, width: "100%", fontSize: 13, padding: "8px 14px" }}
        />
      </div>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        {allCats.map(cat => (
          <button key={cat} onClick={() => setFilterCat(cat)} style={{ padding: "4px 10px", borderRadius: 14, border: `1px solid ${filterCat === cat ? C.blue : C.border}`, background: filterCat === cat ? `${C.blue}18` : "transparent", cursor: "pointer", fontFamily: "DM Sans", fontSize: 11, color: filterCat === cat ? C.blue : C.textDim }}>{cat}</button>
        ))}
      </div>
      ...
      {showQuery && (
        <Card style={{ marginBottom: 16, borderColor: "#A78BFA" }}>
          <SectionTitle>🔍 Consulta de gastos</SectionTitle>
          <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 12 }}>
            <input placeholder="Lugar / comercio (ej: H-E-B)" value={qPlace} onChange={e => setQPlace(e.target.value)} style={inputStyle} />
            <input placeholder="Concepto / tag (ej: Argo)" value={qConcept} onChange={e => setQConcept(e.target.value)} style={inputStyle} />
          </div>
          ...
          {queryResults.length > 0 && (
            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10 }}>
              ...
              <div style={{ maxHeight: 200, overflow: "auto" }}>
                {queryResults.slice(0, 20).map((e, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>
                    <span style={{ fontFamily: "JetBrains Mono", color: C.textDim, marginRight: 8 }}>{fmtDate(e.expense_date)}</span>
                    <span style={{ fontFamily: "DM Sans", color: C.text, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.concept}</span>
                    <span style={{ fontFamily: "JetBrains Mono", color: C.red, marginLeft: 8 }}>{fmt(Number(e.amount))}</span>
                  </div>
                ))}
                {queryResults.length > 20 && <p style={{ fontFamily: "DM Sans", fontSize: 11, color: C.textDim, marginTop: 4 }}>...y {queryResults.length - 20} más</p>}
              </div>
            </div>
          )}
      ...
      {showForm && (
        <Card style={{ marginBottom: 16, borderColor: C.accent }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
            <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} style={inputStyle}>{cats.map(c => <option key={c} value={c}>{c.charAt(0).toUpperCase() + c.slice(1)}</option>)}</select>
      ...
      {showTagging && (
        <>
          <div onClick={() => setShowTagging(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 9999 }} />
          <div style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, zIndex: 10000, minWidth: mob ? "85vw" : 300 }}>
            <h3 style={{ fontFamily: "DM Sans", fontSize: 15, fontWeight: 600, color: C.text, marginBottom: 12 }}>Asociar gasto</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {TAG_OPTIONS.map(tag => (
                <button key={tag} onClick={() => handleTag(showTagging, tag)} style={{ padding: "8px 14px", background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8, cursor: "pointer", fontFamily: "DM Sans", fontSize: 13, color: C.text, textAlign: "left" }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = C.accent} onMouseLeave={e => e.currentTarget.style.borderColor = C.border}>{tag}</button>
              ))}
              <input placeholder="Otro tag personalizado..." onKeyDown={e => { if (e.key === "Enter" && e.target.value) handleTag(showTagging, e.target.value); }} style={{ ...inputStyle, marginTop: 4 }} />
              <button onClick={() => handleTag(showTagging, null)} style={{ padding: "8px 14px", background: "transparent", border: `1px solid ${C.border}`, borderRadius: 8, cursor: "pointer", fontFamily: "DM Sans", fontSize: 12, color: C.textDim, marginTop: 4 }}>Quitar asociación</button>
            </div>
          </div>
        </>
      )}
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        {dailyExpenses.length === 0 ? (
          <div style={{ textAlign: "center", padding: 40 }}><div style={{ fontSize: 36, marginBottom: 12 }}>📝</div><p style={{ fontFamily: "DM Sans", fontSize: 15, color: C.textDim }}>Aún no hay gastos registrados</p></div>
        ) : mob ? (
          <div style={{ maxHeight: "55vh", overflow: "auto" }}>
            <div style={{ position: "sticky", top: 0, zIndex: 10, background: C.surface, padding: "8px 12px", borderBottom: `2px solid ${C.border}` }}>
              <span style={{ fontFamily: "DM Sans", fontSize: 11, color: C.textDim }}>{sorted.length} gastos</span>
            </div>
            {sorted.map((e, i) => {
              const pay = isPayment(e);
              return (
                <div key={e.id || i} style={{ padding: "8px 12px", borderBottom: `1px solid ${C.border}` }} onClick={() => e.id && setShowTagging(e.id)}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                    ...
                    {e.tag && <Badge color="#A78BFA" style={{ fontSize: 9 }}>{e.tag}</Badge>}
          ...
          <div style={{ maxHeight: "65vh", overflow: "auto" }}>
            <div style={{ position: "sticky", top: 0, zIndex: 10, background: C.surface, display: "grid", gridTemplateColumns: "85px 1fr 95px 60px 85px 55px 80px", gap: 6, padding: "10px 12px", borderBottom: `2px solid ${C.border}` }}>
              {Object.entries(sortLabels).map(([col, label]) => (
                <button key={col} onClick={() => doSort(col)} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "DM Sans", fontSize: 11, fontWeight: 600, color: sortCol === col ? C.accent : C.textDim, textTransform: "uppercase", letterSpacing: 0.5, display: "flex", alignItems: "center", gap: 3, padding: 0, justifyContent: col === "amount" ? "flex-end" : "flex-start" }}>{label}{sortCol === col && <span style={{ fontSize: 10 }}>{sortDir === "asc" ? "▲" : "▼"}</span>}</button>
              ))}
              <span style={{ fontFamily: "DM Sans", fontSize: 11, fontWeight: 600, color: C.textDim, textTransform: "uppercase" }}>Tag</span>
            </div>
            {sorted.map((e, i) => {
              const pay = isPayment(e);
              return (
                <div key={e.id || i} style={{ display: "grid", gridTemplateColumns: "85px 1fr 95px 60px 85px 55px 80px", gap: 6, padding: "7px 12px", alignItems: "center", cursor: "pointer" }}
                  onClick={() => e.id && setShowTagging(e.id)}
                  ...
                  <CardLogo source={e.source} />
                  {e.tag ? <Badge color="#A78BFA" style={{ fontSize: 9 }}>{e.tag}</Badge> : <span style={{ fontFamily: "DM Sans", fontSize: 10, color: C.textMuted }}>—</span>}
                </div>
              );
            })}
          </div>
