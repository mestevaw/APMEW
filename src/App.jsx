...
const DRIVE_ROOT_FOLDER = "0B9ZOcVkjNKRIYndnQmlFaFJoWjQ";
const DRIVE_RESOURCE_KEY = "0-vYqYv5R5_d6msUcjKmoSOw";
...
const supaDelete = async (table, id) => {
  await fetch(`${SUPABASE_URL}/rest/v1/${table}?id=eq.${id}`, { method: "DELETE", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, Prefer: "return=minimal" } });
};
const supaUpsert = async (table, data) => {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, { method: "POST", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, "Content-Type": "application/json", Prefer: "return=representation,resolution=merge-duplicates" }, body: JSON.stringify(data) });
  return res.json();
};

// ─── Formatters & Hooks ───
const fmt = (n, d = 0) => { if (n == null || isNaN(n)) return "$0"; return new Intl.NumberFormat("es-MX", { style: "currency", currency: "MXN", minimumFractionDigits: d, maximumFractionDigits: d }).format(n); };
...
const useGoogleDrive = () => {
  ...
  const listFiles = async (folderId, pageToken) => {
    ...
    let url = `https://www.googleapis.com/drive/v3/files?q='${folderId}'+in+parents+and+trashed=false&fields=nextPageToken,files(id,name,mimeType,webViewLink,iconLink,modifiedTime,size,parents)&pageSize=100&orderBy=folder,name&key=${GOOGLE_CLIENT_ID.split("-")[0]}`;
    if (pageToken) url += `&pageToken=${pageToken}`;
    const res = await fetch(url, { headers: { Authorization: `Bearer ${token}`, "X-Goog-Drive-Resource-Keys": `${folderId}/${DRIVE_RESOURCE_KEY}` } });
    return res.json();
  };
...
const Btn = ({ children, onClick, color = C.accent, outline, small, disabled, style: s }) => (<button onClick={onClick} disabled={disabled} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: small ? "6px 12px" : "10px 20px", background: outline ? "transparent" : disabled ? C.surface2 : color, border: outline ? `1px solid ${C.border}` : "none", borderRadius: 8, cursor: disabled ? "default" : "pointer", fontFamily: "DM Sans", fontSize: small ? 12 : 14, fontWeight: 600, color: outline ? C.textDim : disabled ? C.textMuted : C.bg, opacity: disabled ? .5 : 1, transition: "all 0.2s", ...s }}>{children}</button>);
...
const Spinner = () => (<div style={{ width: 18, height: 18, border: `2px solid ${C.border}`, borderTopColor: C.accent, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />);
...
// ═══════════════════════════════════════════
// PAGES (Dashboard, CrudPage, Expenses, Patrimony, Checklist, Projection, DailyExpenses stay same)
// ═══════════════════════════════════════════
...
const DashboardPage = ({ data, mob }) => {
  ...
  const cd = checklist.filter(c => c.is_completed).length; const ct = checklist.length;
...
const CrudPage = ({ title, subtitle, table, items, columns, formFields, defaults, mob, reload, totalLabel, totalKey }) => {
  ...
  const handleSave = async () => { const cleaned = { ...form }; formFields.forEach(f => { if (f.type === "number" && cleaned[f.key] !== undefined) cleaned[f.key] = Number(cleaned[f.key]) || 0; }); ["id","created_at","updated_at","retirement_years","sort_order","currency","is_active"].forEach(k => delete cleaned[k]); if (modal.mode === "edit") await supaUpdate(table, modal.row.id, cleaned); else await supaInsert(table, cleaned); setModal(null); reload(); };
  const handleDelete = async () => { if (confirm("¿Seguro?")) { await supaDelete(table, modal.row.id); setModal(null); reload(); } };
  const total = totalKey ? items.reduce((s, i) => s + Number(i[totalKey] || 0), 0) : null;
  return (<div><h1 style={{ fontFamily: "DM Sans", fontSize: mob ? 20 : 24, fontWeight: 700, color: C.text, marginBottom: 4 }}>{title}</h1><p style={{ fontFamily: "DM Sans", fontSize: mob ? 12 : 14, color: C.textDim, marginBottom: 20 }}>{subtitle}</p><div style={{ marginBottom: 16 }}><Btn onClick={openAdd}>{I.plus} Agregar</Btn></div><Card><Table columns={columns} data={items} onEdit={openEdit} mob={mob} />{total !== null && (<div style={{ display: "flex", justifyContent: "flex-end", padding: "14px 14px 0", borderTop: `1px solid ${C.border}`, marginTop: 8 }}><div style={{ display: "flex", gap: 16, alignItems: "center" }}><span style={{ fontFamily: "DM Sans", fontSize: mob ? 12 : 14, fontWeight: 600, color: C.textDim }}>{totalLabel}</span><span style={{ fontFamily: "JetBrains Mono", fontSize: mob ? 17 : 20, fontWeight: 600, color: C.accent }}>{fmt(total)}</span></div></div>)}</Card>{modal && <Modal title={modal.mode === "edit" ? "Editar" : "Agregar"} fields={formFields} values={form} onChange={(k, v) => setForm(p => ({ ...p, [k]: v }))} onSave={handleSave} onDelete={modal.mode === "edit" ? handleDelete : null} onCancel={() => setModal(null)} mob={mob} />}</div>);
};
...
const ExpensesPage = ({ expenses, categories, mob, reload }) => {
  ...
  return (<div><h1 style={{ fontFamily: "DM Sans", fontSize: mob ? 20 : 24, fontWeight: 700, color: C.text, marginBottom: 4 }}>Gastos en Retiro</h1><p style={{ fontFamily: "DM Sans", fontSize: mob ? 12 : 14, color: C.textDim, marginBottom: 20 }}>Proyección de gastos mensuales</p><div style={{ display: "grid", gridTemplateColumns: mob ? "repeat(2,1fr)" : "repeat(auto-fit,minmax(140px,1fr))", gap: mob ? 8 : 12, marginBottom: 20 }}>{grouped.map((g, i) => (<Card key={i} delay={i * .05} style={{ padding: "12px 14px", textAlign: "center" }}><div style={{ fontSize: 20, marginBottom: 4 }}>{g.icon}</div><div style={{ fontFamily: "DM Sans", fontSize: 11, color: C.textDim, marginBottom: 4 }}>{g.name}</div><div style={{ fontFamily: "JetBrains Mono", fontSize: mob ? 14 : 16, fontWeight: 600, color: C.accent }}>{fmt(g.subtotal)}</div></Card>))}<Card delay={.3} style={{ padding: "12px 14px", textAlign: "center", borderColor: C.accent }}><div style={{ fontSize: 20, marginBottom: 4 }}>💰</div><div style={{ fontFamily: "DM Sans", fontSize: 11, color: C.textDim, marginBottom: 4 }}>TOTAL</div><div style={{ fontFamily: "JetBrains Mono", fontSize: mob ? 14 : 16, fontWeight: 600, color: C.red }}>{fmt(total)}</div></Card></div>{grouped.map((g, gi) => (<Card key={gi} delay={gi * .05} style={{ marginBottom: 14 }}><SectionTitle icon={<span style={{ fontSize: 18 }}>{g.icon}</span>} action={<Btn onClick={() => openAdd(g.id)} small>{I.plus} Agregar</Btn>}>{g.name}</SectionTitle><Table columns={[{ label: "Concepto", key: "concept", bold: true }, { label: "Monto Mensual", key: "monthly_amount", align: "right", mono: true, render: r => fmt(Number(r.monthly_amount)) }, { label: "Notas", key: "notes", color: () => C.textDim }]} data={g.items} onEdit={openEdit} mob={mob} /><div style={{ display: "flex", justifyContent: "flex-end", paddingTop: 10, borderTop: `1px solid ${C.border}`, marginTop: 8 }}><span style={{ fontFamily: "DM Sans", fontSize: 13, color: C.textDim, marginRight: 12 }}>Subtotal</span><span style={{ fontFamily: "JetBrains Mono", fontSize: 15, fontWeight: 600, color: C.accent }}>{fmt(g.subtotal)}</span></div></Card>))}{modal && <Modal title={modal.mode === "edit" ? "Editar Gasto" : "Nuevo Gasto"} fields={fields} values={form} onChange={(k, v) => setForm(p => ({ ...p, [k]: v }))} onSave={handleSave} onDelete={modal.mode === "edit" ? handleDelete : null} onCancel={() => setModal(null)} mob={mob} />}</div>);
};

const PatrimonyPage = ({ assets, debts, mob, reload }) => {
  const totalA = assets.reduce((s, a) => s + Number(a.current_value || 0), 0); const totalD = debts.reduce((s, d) => s + Number(d.outstanding_balance || 0), 0);
  const aF = [{ key: "name", label: "Nombre" }, { key: "asset_type", label: "Tipo", type: "select", options: [{ value: "real_estate", label: "Inmueble" }, { value: "investment", label: "Inversión" }, { value: "business", label: "Negocio" }, { value: "other", label: "Otro" }] }, { key: "current_value", label: "Valor Actual", type: "number" }, { key: "notes", label: "Notas" }];
  const dF = [{ key: "name", label: "Nombre" }, { key: "outstanding_balance", label: "Saldo Pendiente", type: "number" }, { key: "monthly_payment", label: "Pago Mensual", type: "number" }, { key: "interest_rate", label: "Tasa Interés", type: "number" }, { key: "notes", label: "Notas" }];
  const [modal, setModal] = useState(null); const [form, setForm] = useState({}); const [eT, setET] = useState(null);
  const openAdd = (t) => { setET(t); setForm(t === "asset" ? { name: "", asset_type: "investment", current_value: 0 } : { name: "", outstanding_balance: 0, monthly_payment: 0 }); setModal({ mode: "add" }); };
  const openEdit = (row, t) => { setET(t); setForm({ ...row }); setModal({ mode: "edit", row }); };
  const handleSave = async () => { const tbl = eT === "asset" ? "assets" : "debts"; const d = { ...form }; (eT === "asset" ? aF : dF).forEach(f => { if (f.type === "number" && d[f.key] !== undefined) d[f.key] = Number(d[f.key]) || 0; }); ["id","created_at","updated_at","sort_order","currency"].forEach(k => delete d[k]); if (modal.mode === "edit") await supaUpdate(tbl, modal.row.id, d); else await supaInsert(tbl, d); setModal(null); reload(); };
  const handleDelete = async () => { if (confirm("¿Eliminar?")) { await supaDelete(eT === "asset" ? "assets" : "debts", modal.row.id); setModal(null); reload(); } };
  return (<div><h1 style={{ fontFamily: "DM Sans", fontSize: mob ? 20 : 24, fontWeight: 700, color: C.text, marginBottom: 4 }}>Patrimonio</h1><p style={{ fontFamily: "DM Sans", fontSize: mob ? 12 : 14, color: C.textDim, marginBottom: 20 }}>Activos, inversiones y deudas</p><div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "1fr 1fr 1fr", gap: mob ? 10 : 16, marginBottom: 20 }}><StatCard label="TOTAL ACTIVOS" value={fmt(totalA)} color={C.green} delay={.05} mob={mob} /><StatCard label="TOTAL DEUDAS" value={fmt(totalD)} color={C.red} delay={.1} mob={mob} /><StatCard label="PATRIMONIO NETO" value={fmt(totalA - totalD)} color={totalA - totalD >= 0 ? C.accent : C.red} delay={.15} mob={mob} /></div><Card delay={.2} style={{ marginBottom: 14 }}><SectionTitle icon={I.patrimony} action={<Btn onClick={() => openAdd("asset")} small>{I.plus} Agregar</Btn>}>Activos</SectionTitle><Table columns={[{ label: "Activo", key: "name", bold: true }, { label: "Tipo", key: "asset_type", render: r => <Badge color={r.asset_type === "real_estate" ? C.blue : r.asset_type === "investment" ? C.green : C.accent}>{r.asset_type === "real_estate" ? "Inmueble" : r.asset_type === "investment" ? "Inversión" : r.asset_type === "business" ? "Negocio" : "Otro"}</Badge> }, { label: "Valor Actual", key: "current_value", align: "right", mono: true, render: r => fmt(Number(r.current_value)) }]} data={assets} onEdit={r => openEdit(r, "asset")} mob={mob} /></Card><Card delay={.25}><SectionTitle icon={<span style={{ fontSize: 18 }}>💳</span>} action={<Btn onClick={() => openAdd("debt")} small>{I.plus} Agregar</Btn>}>Deudas</SectionTitle><Table columns={[{ label: "Deuda", key: "name", bold: true }, { label: "Saldo", key: "outstanding_balance", align: "right", mono: true, render: r => fmt(Number(r.outstanding_balance)), color: r => Number(r.outstanding_balance) > 0 ? C.red : C.text }, { label: "Pago Mensual", key: "monthly_payment", align: "right", mono: true, render: r => fmt(Number(r.monthly_payment)) }]} data={debts} onEdit={r => openEdit(r, "debt")} mob={mob} /></Card>{modal && <Modal title={modal.mode === "edit" ? "Editar" : "Agregar"} fields={eT === "asset" ? aF : dF} values={form} onChange={(k, v) => setForm(p => ({ ...p, [k]: v }))} onSave={handleSave} onDelete={modal.mode === "edit" ? handleDelete : null} onCancel={() => setModal(null)} mob={mob} />}</div>);
};
...
const DocumentsPage = ({ documents, mob, reload, drive }) => {
  ...
  // Load folder contents when folder changes or token arrives
  useEffect(() => {
  ...
  // Sync current folder to Supabase
    ...
    try {
      ...
      for (const f of docs) {
        const doc = {
          ...
          google_drive_url: f.webViewLink,
          folder_path: currentPath,
          parent_folder_drive_id: currentFolder,
          mime_type: f.mimeType,
          file_type: getFileExt(f.mimeType),
          category: guessCategoryFromPath(currentPath),
        ...
        if (existing && existing.length > 0) {
          await supaUpdate("documents", existing[0].id, doc);
        } else {
  ...
  const TabBtn = ({ id, label }) => (<button onClick={() => setTab(id)} style={{ padding: "8px 20px", fontFamily: "DM Sans", fontSize: 14, fontWeight: tab === id ? 600 : 400, color: tab === id ? C.accent : C.textDim, background: tab === id ? C.accentGlow : "transparent", border: `1px solid ${tab === id ? C.accent + "40" : C.border}`, borderRadius: 8, cursor: "pointer" }}>{label}</button>);
  ...
  return (<div>
    ...
    {tab === "drive" && (<div>
      {!token ? (
        ...
        {/* File list */}
        <Card>
          {loadingDrive ? (<div style={{ textAlign: "center", padding: 30 }}><Spinner /><p style={{ fontFamily: "DM Sans", fontSize: 13, color: C.textDim, marginTop: 12 }}>Cargando carpeta...</p></div>) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {breadcrumb.length > 1 && (
                <button onClick={() => navigateToBreadcrumb(breadcrumb.length - 2)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "transparent", border: "none", cursor: "pointer", borderRadius: 8, width: "100%" }} onMouseEnter={e => e.currentTarget.style.background = C.surface2} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  {I.back}<span style={{ fontFamily: "DM Sans", fontSize: 14, color: C.textDim }}>.. Regresar</span>
                </button>
              )}
              {folders.map(f => (
                <button key={f.id} onClick={() => navigateToFolder(f.id, f.name)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "transparent", border: "none", cursor: "pointer", borderRadius: 8, width: "100%", textAlign: "left" }} onMouseEnter={e => e.currentTarget.style.background = C.surface2} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              ...
              {docs.map(f => (
                <a key={f.id} href={f.webViewLink} target="_blank" rel="noopener" style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 8, textDecoration: "none", width: "100%" }} onMouseEnter={e => e.currentTarget.style.background = C.surface2} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
    ...
    {tab === "indexed" && (<div>
      <Card>
        {documents.length === 0 ? (
          <div style={{ textAlign: "center", padding: mob ? 30 : 40 }}><div style={{ fontSize: 36, marginBottom: 12 }}>📂</div><p style={{ fontFamily: "DM Sans", fontSize: 15, color: C.textDim }}>Aún no hay documentos indexados</p><p style={{ fontFamily: "DM Sans", fontSize: 13, color: C.textMuted, marginTop: 4 }}>Usa la pestaña Google Drive → Sync para indexar</p></div>
        ) : (
          <Table columns={[
...
export default function App() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [profiles, assumptions, income, retIncome, expenses, expenseCategories, assets, debts, checklist, documents, dailyExpenses] = await Promise.all([
        ...
        supaFetch("checklist_items", { order: "sort_order" }), supaFetch("documents", { order: "folder_path,title" }),
  ...
  const renderPage = () => {
    ...
    switch (page) {
      ...
      case "docs": return <DocumentsPage documents={data.documents} mob={mob} reload={loadData} drive={drive} />;
