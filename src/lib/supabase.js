...
const checkResponse = async (res, action) => {
  if (!res.ok) {
    ...
    console.error(`Supabase ${action} error ${res.status}:`, body);
    throw new Error(`Supabase ${action} failed: ${res.status}`);
  }
  return res;
};
...
// ─── Batch update: actualiza todos los rows que cumplan un filtro ───
...
export const supaBatchInsert = async (table, rows) => {
  ...
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, {
    method: "POST", headers: { ...jsonHeaders, Prefer: "return=representation" }, body: JSON.stringify(rows),
  });
